...
import { EvolutionChain, Evolution } from '@core/evolution.model';
...
export class InfoEvolutionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription
      .add(this.pokemonService.getSpeciesByPokemon(this.pokemon.name)
        .subscribe((_pokemon: Species) => {
          this.pokemonService.getEvolutionChainByPokemon(_pokemon.evolution_chain.url)
            .subscribe((_evolutionChain: EvolutionChain) => {
